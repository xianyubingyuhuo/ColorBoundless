...
- [ ] **第二批 9/15-9/22（用户旅程顺序 2026-09-11 修订：测评 → 校色 → 试妆）**：**def 17 后端全链已完成（12 题测评 API + 档案 + 试妆校色联动 + 工具⑤，实测通过）→ 待做：cvd 页测评 UI + 校色选择器（R-06 双模式开关）** → def 15 聚合式试妆 → def 16 肤色分析 → def 18 RAG 最小闭环（搭配语料由用户提供）
